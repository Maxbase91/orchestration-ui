...
// ── Every template is a request lifecycle ──────────────────────────────────
// WF-003 (supplier onboarding) and WF-004 (contract renewal) were "side
// processes" with their own designer screen. Nothing ever started one; they
// were retired on 2026-09-25 — onboarding is a stage inside a request, and a
// renewal comes in through Door 1 (the contract check finds the expiring
// contract). What is left must each define a channel's lifecycle.
console.log('\nEvery template defines a buying channel\u2019s lifecycle');
{
  const unclaimed = workflowTemplates.filter((t) => (t.channels ?? []).length === 0);
  if (unclaimed.length) bad('every template claims a buying channel', unclaimed.map((t) => t.id).join(', '));
  else ok(`all ${workflowTemplates.length} templates claim a channel`);
  const sideTypes = workflowTemplates.filter((t) => ['onboarding', 'renewal'].includes(t.type));
  if (sideTypes.length) bad('no side-process template remains', sideTypes.map((t) => t.id).join(', '));
  else ok('no side-process template remains');
