...
| AI Agent Configuration | Agent library, config form, test panel, performance dashboard. The **Status Answers** agent configures what status questions may be answered: per object, every attribute of the data (label, in the answer / when asked / off, who may see it; new attributes appear switched off) and a role × object matrix (None / Own / All); its test panel asks as any role against live data |
