...
function isPolicyConfig(value: unknown): value is PolicyConfig {
  ...
  if (candidate.minCompetitiveQuotes < 1 || candidate.catalogueMinContentMatches < 1) return false;
  if (candidate.contractUtilisationHeadroom > 100 || candidate.preferredMinPerformance > 100) return false;
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    if (req.method === 'GET') {
      const rows = await sql.query('SELECT config, updated_by, created_at, updated_at FROM procurement_policy_configs WHERE singleton_key = $1', ['default']);
