...
function StepIcon({ status }: { status: OnboardingStep['status'] }) {
  if (status === 'complete') {
    return (
      <div className="flex size-8 items-center justify-center rounded-full bg-ok-soft">
  ...
  if (status === 'in-progress') {
    return (
      <div className="flex size-8 items-center justify-center rounded-full bg-accent-soft">
        <Loader2 className="size-4 text-accent-solid animate-spin" />
      </div>
    );
  }
  return (
    <div className="flex size-8 items-center justify-center rounded-full bg-idle-soft">
...
export function PortalOnboarding() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-accent-line bg-accent-soft/30">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.id} className="relative flex gap-4 pb-2">
              ...
              {!isLast && (
                <div
                  className={cn(
                    ...
                    step.status === 'complete' ? 'bg-ok' : 'bg-border'
                  )}
                />
              )}
              ...
              <Card className={cn(
                'flex-1 py-3 mb-3',
                step.status === 'in-progress' && 'border-accent-line bg-accent-soft/30',
              )}>
                <CardHeader className="pb-1">
                  <CardTitle className="text-sm flex items-center gap-2">
                    {step.label}
                    {step.status === 'in-progress' && (
                      <span className="rounded-full bg-accent-soft px-2 py-0.5 text-xs text-accent-solid font-normal">
                        In Progress
                      </span>
                    )}
                  </CardTitle>
