        ...
        return (
          ...
          >
            <div className="mt-0.5 relative">
              <div className="flex size-6 items-center justify-center rounded-full bg-idle-soft text-[10px] font-medium text-ink-2">
                {c.authorInitials || c.authorName.slice(0, 2).toUpperCase()}
              </div>
              {unread && (
                <span className="absolute -right-0.5 -top-0.5 size-2 rounded-full bg-accent-solid" />
              )}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs text-ink-3">
                <span className={unread ? 'font-semibold text-ink' : 'font-medium text-ink'}>
                  {c.authorName}
                </span>
                {' · '}
                <span className="font-mono">{c.requestId}</span>
                {c.stage && (
                  <span className="ml-1 inline-flex items-center rounded bg-idle-soft px-1 py-0 text-[10px] text-ink-2">
                    {c.stage}
                  </span>
                )}
                {' · '}
              ...
              <p className={`mt-0.5 truncate text-sm ${unread ? 'text-ink' : 'text-ink-2'}`}>
                <AtSign className="inline size-3 text-accent-solid" /> {c.content}
              </p>
            </div>
          </button>
        );
