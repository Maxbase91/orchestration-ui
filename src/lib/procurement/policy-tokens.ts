...
export const POLICY_KEY_META: Record<NumericPolicyKey, { label: string; help: string; unit: '€' | '%' | '/100' | 'days' | ''; usedIn: string }> = {
  ...
  contractRequiredThreshold: { label: 'Contract-required threshold', help: 'At/above this value a PO needs an executed contract behind it', unit: '€', usedIn: 'Your buying channel — the contract-before-PO check · Analytics — spend anomalies' },
  budgetApprovalThreshold: { label: 'Budget approval threshold', help: 'Above this value the demand exceeds standard budget approval and needs VP sign-off', unit: '€', usedIn: 'Your buying channel — the budget approval check · Analytics — spend anomalies' },
