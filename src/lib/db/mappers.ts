...
export function mapDbToWorkflowTemplate(row: DbRecord): WorkflowTemplate {
  return {
    ...
    requesterHeadline: (row.requester_headline ?? undefined) as string | undefined,
    requesterDescription: (row.requester_description ?? undefined) as string | undefined,
  };
}
...
export function mapWorkflowTemplateToDb(w: Partial<WorkflowTemplate>): DbRecord {
  ...
  if (w.requesterHeadline !== undefined) out.requester_headline = w.requesterHeadline;
  if (w.requesterDescription !== undefined) out.requester_description = w.requesterDescription;
  return out;
}
...
const REQUEST_FIELD_MAP: Record<string, string> = {
  ...
  supplierOverrideReason: 'supplier_override_reason',
  approvalChain: 'approval_chain',
  inherentRiskTier: 'inherent_risk_tier',
  materialityTier: 'materiality_tier',
  riskAssessmentRequired: 'risk_assessment_required',
