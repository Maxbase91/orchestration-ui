...
| INT-10 / UX-02 | Adaptive staged intake, one standardised requester experience | `src/features/requests/new-request/`, `src/lib/procurement/demand-conversation.ts`, `src/features/dashboard/dashboard-page.tsx` (home-demand handoff and direct route evaluation) | `npm run test:requester-entry-ui`, `npm run test:intake-guidance-ui` | ADR-0008, `README.md`, `docs/testing/TEST_PLAYBOOK.md` |
| INT-11 / UX-06 | Unified AI-guided demand intake | `src/features/requests/new-request/conversation/` (`intake-conversation.tsx`, `classify-demand.ts`, `use-route-checks.ts`, `call-off-agenda.ts`, `use-service-description-conversation.ts`, `conversation-rules.ts`, `request-rows.ts`), `src/features/dashboard/components/smart-command-bar.tsx`, `src/lib/procurement/intake-seed.ts`, `src/lib/procurement/commodity-candidates.ts`, `api/_domains/intake-upload.ts`, `api/_domains/commodity-match.ts`, `src/lib/workflow/engine.ts` | `npm run test:unified-intake`, `npm run test:demand-conversation`, `npm run test:intake-guidance`, `npm run test:intake-conversation`, `npm run test:ui` | ADR-0005, `src/lib/procurement/SERVICE_DESCRIPTION.md`, `docs/testing/TEST_PLAYBOOK.md` | Broad Goods/Services values remain internal; upload extraction and contextual guidance are best-effort and fall back deterministically. |
| UX-04 | One configurable home — per-role default layout, persisted widget customisation | `src/features/dashboard/widget-registry.tsx`, `src/stores/dashboard-store.ts` | `npm run test:dashboard-widgets`, `npm run test:dashboard-ui` | `README.md`, `docs/testing/TEST_PLAYBOOK.md` |
...
| CLS-01/02/03 | Classification, commodity-code assignment, confidence handling | `src/lib/procurement/classify.ts`, `src/lib/procurement/category-code.ts` (codes configured per category in `/admin/categories`), `api/ai.ts` | `npm run test:classification-eval`, `npm run test:category-code`, `npm run test:ai-agents` | `README.md`, `docs/testing/TEST_PLAYBOOK.md` |
...
| CHK-07 / CON-05 | Contract-aware demand matching and adaptive clarification | `src/lib/procurement/contract-matching.ts`, `api/_domains/contract-match.ts`, `api/_domains/contract-scope.ts`, `src/features/contracts/contract-detail-page.tsx`, `contract_scope_*` tables and curated Neon backfill | `npm run test:contract-matching`, `npm run test:contract-match-api`, `npm run test:governed-checkout-atomic` | ADR-0004, `docs/specs/requirements/07-contracts.md` | Deterministic rule gate is active; provider reranking is optional and scope metadata must remain maintained. |
...
| SUP / RTE-04 | Supplier directory, profile, screening state and conditional onboarding | `src/features/suppliers/`, `src/lib/procurement/supplier-data.ts`, `src/features/suppliers/portal/portal-onboarding.tsx`, `src/features/suppliers/components/profile-risk-tab.tsx`, onboarding stages | `npm run test:supplier-data`, `npm run test:onboarding-stage`, `npm run test:screening`, `npm run test:ui-lifecycle` | `README.md`, `src/features/suppliers/README.md`, `docs/specs/requirements/04-supplier-management.md` | Supplier-owned company/contact submission and rationale-gated vendor-manager/admin controls are present; durable decision audit and requester-specific forms remain to be validated live. |
...
| AST-Q/P/S/X | Assistant lookup, grounded knowledge, support handoff and conversation history | `src/lib/assistant/`, `api/chat.ts`, `api/execute-action.ts`, `src/lib/db/tickets-core.ts` | `npm run test:knowledge`, `npm run test:assistant-intents`, `npm run test:tickets`, `npm run test:ticket-sla`, `npm run test:execute-action`, `npm run test:shared-core` | `docs/specs/requirements/11-ai-assistant-knowledge.md` |
...
| INT-12 / LIF-01 | Atomic full-intake submission and first-stage transition | `api/_domains/intake-submit.ts`, `src/lib/procurement/submit-intake.ts`, dispatcher routing through `api/db.ts?domain=` with `vercel.json` rewrites; request, service-description, compliance, stage-history, workflow-instance and approval-entry writes are transactional; request inserts tolerate additive schema rollout gaps; legacy orphan intake rows are repaired only on a safe retry | `npm run test:intake-submit`, `npm run test:api-domain-routing`, `npm run test:api-imports`, `npm run test:ui` | `docs/testing/TEST_PLAYBOOK.md`, lifecycle ADRs | Completed intake now enters the shared validation gate; Neon live transaction tests require reachable database DNS. |
...
| UX-NAV-01 | Role-aware deep links from dashboards and request details | `src/App.tsx` detail-route guards, `src/features/requests/request-detail/`, `src/features/dashboard/widgets/widget-expiring-contracts.tsx`, requester read-only detail controls | `npm run test:link-route-integrity`, `npm run test:link-navigation` | `README.md`, `docs/testing/TEST_PLAYBOOK.md`, feature READMEs | Requesters can inspect supplier and contract detail; purchasing and governance mutations remain restricted. |
