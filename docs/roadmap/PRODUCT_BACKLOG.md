...
| 8.1.2 | As a **Requester**, I want accounting defaults filled from my profile, not asked for. | Cost centre and beneficiary come from `procurement_profiles`. The cost centre is **shown, not asked** — derived and correctable in one place (the requester-context block) — and a checkout asks for it only when the profile genuinely has no default. It used to be a dropdown of five **invented** cost centres in one checkout and free text in another, while the governed checkout overrode both with the profile value. | 🟢 |
...
| OB-11 | A cost-centre reference source | `requests.cost_centre` is free text and there is no reference table. The value comes from the requester's profile; an organisation with a real chart of accounts will want it validated. | M |
