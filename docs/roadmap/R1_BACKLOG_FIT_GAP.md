...
| CLS-B2 | Classification aggregation API | 🟡 | `/api/ai.ts` reads the admin-configured classifier and returns a controlled 503 when server-side Supabase configuration is absent (`test:ai-api-config`); Production still requires `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, and a redeploy to enable the live classifier |
