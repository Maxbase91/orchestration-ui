...
| CLS-B2 | Classification aggregation API | 🟢 | `/api/ai.ts` reads the admin-configured classifier and returns a controlled 503 when server-side Supabase configuration is absent (`test:ai-api-config`). Production has the required Supabase configuration, the supported configurable Groq default (`openai/gpt-oss-20b`), and an active classifier verified by `test:ai-agents` (16 passed, 0 failed). |
...
> **Release security gate — not yet implemented.** Current role switching is browser state and the
> prototype's Supabase policies are permissive; neither is authorization. Before an R1 deployment,
> introduce Supabase Auth, map authenticated identities to the directory, derive roles server-side,
> replace permissive RLS policies with ownership/role predicates, and require authenticated API
> sessions. This is deliberately recorded as a full security workstream rather than a partial policy
