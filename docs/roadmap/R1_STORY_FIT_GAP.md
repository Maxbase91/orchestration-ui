...
> those ids**: commit messages, `TEST_PLAYBOOK.md` cases, and code comments. The Definition of Done
> (then in CLAUDE.md, now AGENTS.md rule 6) still said to "flip the affected roadmap story/epic status
> when a gap closes", which was not possible with nothing left to flip.
>
...
| CFG-04 | PSL & competitive sourcing rule management | 🟢 | Competitive-sourcing threshold + min-quotes + preferred-supplier performance bar in `policy-config.ts`. **Hard PSL reference list + admin editor done (2026-09-23):** `category_preferred_suppliers`, maintained per category in `/admin/categories` beside its managers; each category also carries the supplier tags that recognise a supplier as covering it. The list decides "preferred"; the performance heuristic answers only for a category with no list. Seeded from what the heuristic decided (`backfill:category-psl`). The competitive-sourcing exempt categories are a Decisioning threshold (2026-09-24), edited as a checklist at `/admin/thresholds`. The **direct call-off limit** is a Decisioning threshold too (2026-09-24), enforced at checkout and in the intake conversation — the route decision rules a contract out when the estimate is already over it, and a call-off value over it is refused. **Preferred-supplier enforcement (2026-09-24):** a supplier outside a category's non-empty list needs a reason (asked in the conversation's supplier question, required by submit, which recomputes the override from `category_preferred_suppliers`), and — a Decisioning switch, on by default — adds the category manager to the approvals when the chain lacks them; the reason shows beside the supplier on the request |
...
| Story | Summary | State | Note |
|---|---|---|---|
| CLS-01 | AI classification + catalogue/contract sufficiency | 🟡 | Real LLM (Groq/Gemini); now validates against the **configured taxonomy** (not a separate hardcoded list). The deterministic fallback classifier is centralised in `lib/procurement/classify.ts` (single source of truth, benchmarked) — and now genuinely single: the assistant's private `guessCategory` keyword table has been removed, so the assistant and intake can no longer disagree about the same sentence. An LLM category outside the configured taxonomy falls back to that classifier rather than to a literal `goods`, which is catalogue-eligible and would have re-opened CHK-01's defect from the other side |
| CLS-02 | Category-code assignment & taxonomy translation | 🟢 | `lib/procurement/category-code.ts` — keyword → standardised code, **category-aware** with **per-category default codes** so every demand resolves. **Codes are category configuration (2026-09-24):** each category carries its own codes (code, label, keywords) and default, edited in `/admin/categories` → Commodity codes, read by intake and by the commodity-match endpoint from the store — a deployment brings its own scheme without a code change. Keywords match at the start of a word (substring matching coded "campaign" and "maintenance" as data analytics); ties go to the demand's own category. Seeded by `backfill:category-commodity-codes` |
...
| CHK-03 | PSL enforcement at check | 🟡 | The check reads the category's **preferred-supplier list** (`/admin/categories`); every preferred supplier is **invited when a sourcing event is created** (`sourcing-invitees.ts`); the supplier and the category's preferred suppliers are shown on the intake panel and the request. Still a soft check: an override needing a reason + category-manager approval is not built |
...
| RSK-06 | Detailed assessment handoff (no front-door capture) | 🟡 | `lib/procurement/handoff.ts` — when reuse isn't possible the detailed assessment is a **routed step to the risk register** (deep-link, no front-door capture); listed under Next steps in the Channel page's workings |
...
| DET-04 | **Determination screen** (the R1 endpoint) | 🟢 | Now the **Channel page** (2026-09-26, replacing Review & submit, for full requests and contract call-offs): how it will be bought in the channel template's words; every stage of the template with *Applies / If … / Skipped* and why, from the server's landing rule and the engine's own branch walk (`lib/workflow/channel-plan.ts`); what the requester does at each stage; the approvers submit will write; who sourcing will invite; the checks as verdicts (`lib/procurement/channel-checks.ts`); and **How this was worked out** — materiality, inherent and operational risk, contract/sourcing type, approval-to-source, Next-steps handoff (system/status/deep-link), every policy check. **Exportable** (`Export` → structured Markdown via `determination-export.ts`). The risk *questions* moved to Details — every question is asked before any conclusion is shown. The determination itself is a pure module (`lib/procurement/intake-determination.ts`), computed once per intake and shared by both densities |
| DET-05 | Approval-to-source: pre-sourcing validation gate | 🟢 | `lib/procurement/approval-to-source.ts` — standardised gate: **light** (demand validation + cost-centre) vs **full** (demand validation + intent-to-source + category approval), triggered by value ≥ threshold (POL-01), materiality, or high/critical inherent risk; transactable early exit ⇒ no gate. Surfaced under "Approval to source" in the Channel page's workings + in the export. Threshold seedable in the CFG sim panel |
...
| DET-09 | Sourcing type: new/renewal/benchmarking | 🟢 | `determineSourcingType` — none / renewal / benchmarking (incumbent) / new-event from channel + category + relationship; surfaced on the determination **and now persisted** (`requests.sourcing_type`), so `new-event` survives intake and drives the sourcing stage |
