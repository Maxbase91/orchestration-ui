...
- FR01-29a · **Submit decides again** ([ADR-0010](../../adr/0010-submit-decides-again.md)). The server makes the determination once more from stored data (`api/_determination.ts`) — the supplier, its contracts and its reusable assessments through the same ports the browser reads, the routing rules, approval chains and Request Validator through the same read code, the thresholds from the stored row — with the answers to the risk questions the page sends (`riskAnswers`). It compares what the Channel page showed (`determination-changes.ts`): the channel, approval chain, sourcing type, inherent-risk and materiality tiers, risk assessment, screening, disposition, the SRA outcome, each policy check's result, the reusable assessments, vendor onboarding and the risk questions. A difference is refused with **409 `determination_changed`** and each change in a sentence, and nothing is written; the page fetches its inputs again, stays on the Channel page and says what changed. Otherwise the request and its compliance record carry the server's determination, never the browser's copy (`test:submit-decides-again`, `test:ui`).
