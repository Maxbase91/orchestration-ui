...
| TC-ADM-12 | SLA targets admin | **Read-only.** The workflow template owns stage SLAs — `requests.sla_deadline` is computed from the stage node's `slaDays` (`src/lib/workflow/stage-sla.ts` + `business-days.ts`), not from `sla_targets`. The page shows the template-derived values and links to `/admin/workflows` to change them. `sla_targets` retains only its `stage='ticket'` rows, which drive ticket first-response due dates (`tickets-core.ts`). A regression test fails the build if the page starts writing the table again. |
...
| TC-CFG-12 | Channel lifecycles derive from templates (`npm run test:channel-map-parity`) | The lifecycle was defined twice — `buying-channel-stages.ts` in code, the template graph in data — and they disagreed for every channel. WF-002 carried a phantom `validation` stage catalogue skips and lacked intake, invoice and payment; four channels had no template at all, so there was nothing to derive from. This commit corrects the data and authors WF-005/006/007 while **the code map is still authoritative** — nothing at runtime reads the derivation yet, so the change is provably behaviour-neutral. Asserts the derived map equals `STAGES_BY_CHANNEL` stage-for-stage and in order for all six channels (seed and live), no channel unclaimed or claimed twice, that WF-003/WF-004 claim none (they are workflows for other objects), that traversal follows **edges** not the nodes array (WF-001 lists Risk Assessment third but reaches it after Validation), that a cycle terminates, that two nodes normalising to one stage list it once, and that no open request sits in a stage its channel skips. |
