...
| TC-CAT-PAGE | The Catalogue page (`npm run test:requester-entry-ui`, `npm run test:ui`) | Catalogues are the items' own; items from two suppliers show "Placed as 2 orders"; under the threshold the note says it goes straight to the supplier, over it (on the **basket** total) that the manager approves; Place order waits for deliver to, charged to (active reference rows only) and a purpose; placing sends one basket call with every order decided on the total, names each placed order with what happens next, and empties the basket |
