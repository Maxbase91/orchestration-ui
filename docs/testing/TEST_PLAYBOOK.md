...
| TC-REQ-10b | Add detail on the buy-route screen, then click "Use this detail" | The box **clears**, a confirmation says the options have been re-checked, and the detail appears **once** in the matching text. It used to append to `title` while the box kept its contents — so the demand read "buy business consulting — IT strategy work — IT strategy work", the request was renamed to that run-on, and the click looked ignored |
| TC-REQ-10c | A demand with no detail that has possible contract coverage | The contract option says coverage may exist and asks for the one detail that settles it; the **candidate list is not shown** and no disabled "Confirm details first" buttons appear. Candidates appear once the matcher confirms coverage, or the requester supplies detail. When the matcher has a clarifying question (ADR-0004) that question is what the box asks, not a generic prompt |
| TC-REQ-10d | Press **Add detail** on a ruled-out route | The detail box is revealed, scrolled to and **focused** — the cursor is in it. The handler only set the flag that reveals the box, and that box already renders whenever nothing matched, which is exactly when the button appears: the press changed a condition that was already true and the screen did not move (`test:ui`) |
| TC-REQ-10 | Step 2 **How you'll buy** (one screen, INT-10) | All three routes visible at once — catalogue, contract call-off, full request — recommendation badged and first, each in requester language with an indicative timeline. The wording is the claiming workflow template's **requester headline / description** (Admin → Workflows), not a code table; `test:requester-entry-ui` serves a template with its own headline and asserts the catalogue option shows it. A ruled-out route **states its reason in place** rather than disappearing; every route stays clickable, so a wrong match never hides the right path. Matched words, fit%, contract utilisation and the routing rule id are evidence behind **"Why this?"**, expert density only |
| TC-REQ-11 | Step 3 chat intake | Captures value/timeline/commodity; **Service Description** builds (see Suite SOW) |
| TC-REQ-11e | Details asks for what submit requires (`npm run test:submission-requirements`, `npm run test:ui`) | A full request needs a title, a need-by date that parses and a cost centre — **one list** (`submission-requirements.ts`) that the server's 422 and the Details gate both read. Details holds Next until all three are given and the footer names each with where it is entered (the form, Key facts, Charged to). Charged to reads "Not set yet — needed before you submit", not "you can add it later"; the conversation no longer promises to leave the date open, and a skipped date is entered in Key facts, where it used to be read-only — so such a request could never be submitted. The budget stays optional (the server accepts zero). Approver avatars show at most two initials — a role-open approver by its role ("CM", not "CM—RFoAM") |
...
| TC-REQ-R7 | Catalogue/contract sources unreachable | "We could not check what already exists — neither was checked. Nothing has been ruled in or out", with a route to a full request. Never an endless spinner, and never "no match" for a check that never ran |
...
| TC-INT-C1 | Start a request, press "Browse the catalogue", go Back, and choose "Raise a full request" | Details opens the conversation. It rendered nothing: the shortcut stored the catalogue *route* as the form's *category*, and a catalogue category on the full-request route matched no branch — an empty step whose gate asked for a title and value it had no field for (`test:requester-entry-ui`, `test:unified-intake`) |
...
| TC-REV-1 | Reach Review & submit | The groups read **How you'll buy → Risk → Routing & approvals → Checks we ran**. The channel leads: it is the thing the requester has to understand before submitting, and the group meanings say so. Risk used to render first, contradicting both |
| TC-REV-2 | Review a demand with no supplier selected | The screen must not claim contract coverage. `runSecondContractCheck` requires a supplier — without one it matched **any** contract in the category, so the screen said "a usable contract covers this demand — transact directly" about an agreement with a company nobody had chosen, while also saying "No existing contract found" and "no existing agreement". Worse, that stray `transact` fed `earlyExit` and **switched off the approval-to-source gate** |
| TC-REV-3 | Read the inherent-risk tier on Review | It appears **once**, under Risk. It was stated there and again inside the channel card |
...
| TC-STEP-1 | Start a contract call-off | The stepper shows three steps, not four. A call-off submits from its checkout and goes straight to confirmation, so a Review step on that route is one the stepper advertises and the requester can never reach — the same reason the catalogue fast track has none |
...
| TC-ADM-12 | SLA targets admin | **Read-only.** The workflow template owns stage SLAs — `requests.sla_deadline` is computed from the stage node's `slaDays` (`src/lib/workflow/stage-sla.ts` + `business-days.ts`), not from `sla_targets`. The page shows the template-derived values and links to `/admin/workflows` to change them. `sla_targets` now holds **only** its `stage='ticket'` rows, which drive ticket first-response due dates (`tickets-core.ts`): the nine stage rows disagreed with the templates in six of nine stages while nothing read them, and were deleted with the orphaned `src/lib/db/sla-targets.ts` + hook (zero importers repo-wide). `test:config-consumption` fails the build if a stage row reappears, if either module returns, or if the page starts writing the table again. |
...
| TC-ADM-12d | The template follows the channel (`npm run test:workflow-steps`) | Submit takes the template that claims the request's channel and ignores any the browser sends; intake no longer derives a template from the category (that rule gave WF-001 to nearly every category, so a business-led request would have run the procurement-led lifecycle). Review previews the channel's template; a call-off's workflow instance is created by the checkout only — the browser used to add a second one on the category's template |
...
| TC-ADM-32 | Audit Export produces a file | `/admin/audit` → Export downloads `audit-log-YYYY-MM-DD.csv` containing the **filtered** rows (not the visible page, not the whole table), disabled when the filters match nothing. The button previously had no `onClick` at all, on the screen whose purpose is producing evidence. One shared `src/lib/csv.ts` replaces the two divergent copies in the analytics pages, and writes a UTF-8 BOM so Excel does not mangle every `€` and umlaut. `test:csv-export` |
...
| TC-ADM-38 | Side processes have their own surface | `/admin/workflows` lists only the five templates that define a buying channel's lifecycle; `/admin/workflows/side-processes` lists WF-003 (Supplier Onboarding) and WF-004 (Contract Renewal), which govern a supplier and a contract and are selected by category. The two screens **partition** the templates — one component, one predicate (`isSideProcess`), two routes — so neither can drop a template or show it twice. Each carries its own banner: the request one says the graph decides a request's stages, owners and SLAs, which was being shown over side processes where it is false. The buying-channel selector is hidden on the side-process scope, because claiming a channel is exactly what would stop it being one. `test:channel-stages` also asserts the converse: every request lifecycle claims a channel, and no side process does. |
| TC-ADM-37 | Every editor that claims to save, saves | `test:admin-editors` covers **10 surfaces**, not the 4 it began with: routing rules, AI agents, workflow templates, forms, approval chains, categories, cost centres, delivery locations, knowledge base and service descriptions — plus the policy-config singleton and the category-manager set, which are not row editors. Two halves per surface, and both are needed: a **live round trip** (write, read back, JSONB columns still arrays — a save that flattens `nodes` to a string round-trips the name while destroying the template) and a **static check** that the Save handler calls the mutation. Either alone passes the defect this exists for: `/admin/database`'s Workflows tab showed a green "Live (persisted)" badge, wrote an audit row claiming `record.update`, and had no persistence branch. `knowledge_base` and `service_description_templates` are **empty live** — everything falls back to the built-in sets, which is documented behaviour — so the suite inserts a row, round-trips it and removes it, because an empty table is exactly where a broken editor hides. The three deliberately read-only pages (SLA targets, system health, policy management) are asserted to stay that way. |
...
| TC-DET-05a | Open Details on the chat path | Only the requester summary and the conversation are on screen. Supplier selection appears **after** the conversation finishes, and is never a gate — leaving the supplier open is a valid answer. The step used to render the chat, a risk card and supplier selection at once, before the requester had answered anything (`test:details-progression`, `test:ui`) |
...
| TC-DET-08 | Decline an optional question (say you don't know the need-by date) | Next still opens. Only the mandatory floor and the **triggered risk questions** gate the step; an optional slot the requester declined must not block it — gating on an empty agenda made the chat say "that's everything I need" while Next stayed disabled with nothing on screen to explain why (`test:details-progression`) |
...
| TC-UI-02 | `/requests/new` → describe in free text → Accept & continue | **No commodity-category tiles** (Goods/Contingent Labour asserted absent); the free-text classification derives the category and a **"Browse the catalogue"** affordance is the one explicit alternative entry. The **How you'll buy** screen renders via the connector layer in-browser and a **plain product word** ("laptops") surfaces the model-named catalogue items (regression — scoring is a raw keyword sum, not length-normalised, so a description-level hit counts and extra words don't dilute it); all three routes are asserted present at once, the recommendation is badged, and the matched words are asserted **absent from the surface** and present only after opening "Why this?". Expected dev-only `/api/*` 404s are ignored; no app errors during the flow |
| TC-UI-03 | Inputs then conclusions (Contract Renewal → full request → **step 3 Details** → **step 4 Review & submit**) | Details carries every question — the demand form, the mini-IRQ delta capture with its "asked because" rationale (this is the FORM path, which has no conversation to carry them), and the IT security form for software. Review carries every conclusion and nothing to fill in: buying channel, materiality, inherent risk (driven by the mini-IRQ answered on the previous step), contract/sourcing type, approval-to-source, Next-steps handoff, the routing/lifecycle preview and approvals, and the policy-check region (competitive sourcing/PSL when the validator agent is active, else the notice). Export downloads the determination as Markdown. |
