...
| TC-CFG-01 | Central policy config (`npm run test:policy-config`) | `DEFAULT_POLICY_CONFIG` pins every decisioning threshold (approval/materiality/risk-band/competitive-sourcing/contract); all decisioning modules source their constants from it (values unchanged — dependent suites stay green); `resolvePolicyConfig` merges a partial override without mutating defaults — the seam for admin/sim-panel rule management. |
...
| TC-DET-06 | Demand disposition (`npm run test:referral` + UI smoke) | proceed / request-change / refer-back, most-blocking-wins: missing-mandatory or out-of-scope → refer-back; failed policy check or duplicate → request-change; else proceed. Headline banner renders on the determination and appears in the export. |
...
| TC-UI-05 | Interaction E2E (`npm run test:interactions-ui`, self-cleaning) | **Wizard submit** creates a request (reaches confirmation, persists, then deleted); **admin category create** persists & shows in the table (then deleted); **AI assistant** returns a response within 25s (no hang) **and a supplier lookup returns connector-backed data** (AST-Q — the assistant reads through the same governed ports as the front door). No uncaught errors in any flow. |
