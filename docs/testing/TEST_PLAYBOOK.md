...
| TC-UI-05 | Interaction E2E (`npm run test:interactions-ui`, self-cleaning) | **Wizard submit** creates a request (reaches confirmation, persists, then deleted); **admin category create** persists & shows in the table (then deleted); **AI assistant** returns a response within 25s (no hang) **and a supplier lookup returns connector-backed data** (AST-Q); **config wiring** — an admin lowers the approval threshold to 10k and saves, then the **same €50k demand that is a *light* gate at the default 250k becomes a *full* gate in the live wizard determination** (proves admin config drives the live front door). No uncaught errors in any flow. |
