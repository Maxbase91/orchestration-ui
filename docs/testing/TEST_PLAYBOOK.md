...
npm run test:record-ids           # a new admin record never takes a live record's id: forms, agents, routing rules, knowledge-base entries and criteria are numbered one past the highest id in use
npm run test:supplier-evidence     # a supplier's screening, SRA and onboarding are recorded only on evidence: a screening names its reference and day, the SRA stands on a completed in-date assessment, onboarding completes on a clear screening with its note kept
...
| TC-SOW-10b | A required section is asked, and counted once (`npm run test:demand-conversation`, `npm run test:ui`) | A goods demand at €50k whose description handles personal data is asked for **acceptance criteria and dependencies** — its own slot conditions would ask neither — because the default template requires both at high data sensitivity; the question says the section rule is why. The floor holds until they are answered. `requiredSectionIds` is the one set the panel, the Channel page and the stored record read, and it keeps a mandatory `asked: false` section. Verified by removing the section rule from the agenda and the floor — four checks fail. In the browser, the Channel page's "N of M required sections" must equal the panel's (the smoke answers `/api/generate-sow` with the captured answers so the summary renders); giving the page one section fewer fails it ("panel 4, Channel page 1") |
