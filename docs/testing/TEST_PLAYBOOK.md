...
| TC-REQ-09 | Step 1: edit the classified Category (e.g. →Consulting) | **Override propagates** to Step 3 header, Summary, and the submitted record |
| TC-REQ-10 | Step 2 Pre-check | Catalogue match + ranked contract candidates with fit% + keyword reasons |
...
| TC-UI-02 | `/requests/new` → pick a category → Next | Category tiles render from the **canonical taxonomy** (Goods/Consulting/Software/Services asserted); pre-check step renders (proves catalogue/contract/supplier reads run **through the connector layer in-browser**); no runtime errors during the flow |
| TC-UI-03 | Drive to the determination screen (Contract Renewal → full request → fill value) | Determination screen renders; **materiality + inherent-risk segmentation surface**; **mini-IRQ delta toggle (critical service) drives the cascade to critical live**; **Next-steps handoff panel renders** (systems + status + deep-links); policy-check region renders (DTPS/PSL checks when the Request Validator agent is active, else the validator notice) |
...
| TC-UI-05 | Interaction E2E (`npm run test:interactions-ui`, self-cleaning) | **Wizard submit** creates a request (reaches confirmation, persists, then deleted); **admin category create** persists & shows in the table (then deleted); **AI assistant** returns a response within 25s (no hang). No uncaught errors in any flow. |
