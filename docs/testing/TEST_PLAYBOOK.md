...
| TC-STEP-1 | Start a contract call-off (`npm run test:ui`) | The stepper shows four steps, like a full request: the call-off's Details form continues ("See how it will be bought") to its own Channel page — WF-008's stages landed by the checkout's rule (contracting skipped · contract used as it stands, risk skipped · existing assessment reused, *Applies · over €1,000* on approval), the checks from its governed decision, and **Submit the call-off**. The form used to submit directly under a button that read "Review request" |
...
| TC-UI-03 | Inputs then conclusions (full request → **step 3 Details** → **step 4 Your buying channel**) | Details carries every question — the conversation, its mini-IRQ tail with the "asked because" rationale, and the supplier. The Channel page carries the conclusions and nothing to fill in: the stages that will run and why, the checks, and the workings (materiality, inherent risk driven by the mini-IRQ answer, contract/sourcing type, approval-to-source, next steps) — with Save as draft and Submit |
