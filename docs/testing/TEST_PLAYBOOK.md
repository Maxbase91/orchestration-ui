...
| TC-UI-02 | `/requests/new` → reveal manual grid → pick a category → Next | Free text is the **primary entry**; the category grid is hidden until "choose a category manually" is clicked, then tiles render from the **canonical taxonomy** (Goods/Consulting/Software/Services asserted). Pre-check **stage 1 (catalogue)** renders via the connector layer in-browser; the **contract check is asserted absent** until enrichment; stage 2 (contract) is reached only after enriching. No runtime errors during the flow |
