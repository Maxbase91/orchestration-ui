...
| TC-CC-2 | The same order with a profile that has no cost centre | One plain text field appears, explaining why. There is **no picker of invented cost centres** — `requests.cost_centre` is free text and no reference table exists |
...
| TC-UI-03 | Inputs then conclusions (Contract Renewal → full request → **step 3 Details** → **step 4 Review & submit**) | Details carries every question — the demand form, the mini-IRQ delta capture with its "asked because" rationale, and the IT security form for software. Review carries every conclusion and nothing to fill in: buying channel, materiality, inherent risk (driven by the mini-IRQ answered on the previous step), contract/sourcing type, approval-to-source, Next-steps handoff, the routing/lifecycle preview and approvals, and the policy-check region (competitive sourcing/PSL when the validator agent is active, else the notice). Export downloads the determination as Markdown. |
